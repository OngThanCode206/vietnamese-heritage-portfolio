...
function Portfolio() {
  ...
  // SỬA: Đọc động nhãn "Sở thích" từ t.ui.interestsTitle
  const nav = [
  ...
  return (
    <div className="paper-grain min-h-screen bg-background font-sans text-foreground antialiased">
        ...
        >
          <div className="grid gap-6 md:grid-cols-2">
            {t.activities.map((act, index) => (
              <article key={`${lang}-act-${index}`} className={cardClass}>
      ...
      <Section
        id="so-thich"
        eyebrow={lang === "en" ? "Personal" : lang === "kr" ? "개인" : "Cá nhân"}
        title={t.ui.interestsTitle}
      >
        <div className="grid gap-6 sm:grid-cols-3">
          {t.interests.map((item: string, idx: number) => {
            const icon = item.match(/\p{Extended_Pictographic}/u)?.[0] || "✨";
            const title = item.replace(/\p{Extended_Pictographic}/u, "").trim();

            // SỬA: Từ điển mô tả hỗ trợ chuẩn cả 3 ngôn ngữ
            const multiLangDescriptions: Record<Lang, Record<string, string>> = {
              vi: {
                "Bóng đá": "Theo dõi các trận cầu sôi động, giao lưu và rèn luyện thể lực cùng tinh thần đồng đội.",
                "Đi du lịch": "Trải nghiệm văn hóa, khám phá danh lam thắng cảnh và ghi lại những khoảnh khắc đẹp.",
                "Ăn uống": "Thưởng thức ẩm thực đa dạng, khám phá các quán ăn ngon và đặc sản các vùng miền.",
              },
              en: {
                "Football": "Watch exciting matches, connect with friends, and stay active.",
                "Traveling": "Experience new cultures, explore scenic spots, and capture beautiful moments.",
                "Eating out": "Enjoy diverse cuisine, discover great food spots and local specialties.",
              },
              kr: {
                "축구": "흥미진진한 경기를 관람하고 친구들과 소통하며 건강을 유지합니다.",
                "여행": "새로운 문화를 체험하고 아름다운 풍경을 탐방하며 추억을 남깁니다.",
                "맛집 탐방": "다양한 음식을 즐기고 맛집과 지역 특산물을 탐방합니다.",
              },
            };

            const desc = multiLangDescriptions[lang]?.[title] || multiLangDescriptions.vi[title] || "";

            return (
              <article key={`${lang}-interest-${idx}`} className={cardClass}>
